...
#count matrix
library(edgeR)
counts = read.delim("~/OneDrive/oxford/summer_internship/deseq2/counts_blc.txt", row.names = 1)
rownames(counts) = substr(rownames(counts), 1, 15)
colnames(counts)[2] = c("EXP22BLC03A")
colnames(counts)[3] = c("EXP22BLC04A")
colnames(counts)[6] = c("EXP26BLC03A")
colnames(counts)[7] = c("EXP26BLC04A")
colnames(counts)[10] = c("EXP28BLC03A")
colnames(counts)[11] = c("EXP28BLC04A")
##tidy up
...
cell_line = substr(colnames(counts), 9, nchar(colnames(counts)))
hnf4a_donor = substr(colnames(counts), 9, nchar(colnames(counts)))
same_donor = function(x) {
  x = gsub("SB", "D1", x)
  x = gsub("EX1", "D1", x)
  x = gsub("03A", "D2", x)
...
blc_all = topTable(fit_bayes, p.value = 1, number = Inf)
write.csv(deg, "dream.deg.blc.07122018.csv")
##########################################################
...
table(is.finite(deg$entrez)) #how many conversions are successful? (131 unsuccessful)
...
pro_seq = getSequence(id = deg$entrez,
                      ...
                      upstream = 2000, 
                      mart = mart)
exportFASTA(pro_seq, "blc.pro.fasta.txt")
...
fimo = read.delim("~/OneDrive/oxford/summer_internship/differential_gene_expression/blc.fimo.txt") #import fimo results 
...
#explore and export 
write.csv(deg_hnf4a_t, "blc.hnf4a.target.all.csv") #all (regardless of direction)

library(dplyr)
deg_hnf4a_t_down = filter(deg_hnf4a_t, logFC < 0) #downregulated genes
write.csv(deg_hnf4a_t_down, "blc.hnf4a.target.down.csv")

library(dplyr)
deg_hnf4a_t_up = filter(deg_hnf4a_t, logFC > 0) #upregulated genes
write.csv(deg_hnf4a_t_up, "blc.hnf4a.target.up.csv")
